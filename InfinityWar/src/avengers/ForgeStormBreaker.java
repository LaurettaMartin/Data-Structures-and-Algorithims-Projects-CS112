...
public class ForgeStormBreaker {
    public static void main (String [] args) {
        ...
        int numOfCols = StdIn.readInt();
        int sumOfFluxIntensityData = 0;

        int[][] fluxIntensityData = new int [numOfRows][numOfCols];
        boolean[][] array = new boolean [numOfRows][numOfCols];
        ...
        // for(int row = 0; row < fluxIntensityData.length; row++){
        //     for(int col= 0; col < fluxIntensityData[0].length; col++){
        //         sumOfFluxIntensityData += fluxIntensityData[row][col];
        //     }
        // } sumOfFluxIntensityData++;
        
        for(int row = 0; row < nums.length; row++){
                sumOfFluxIntensityData += nums[row];
            }
        
        // Set the output file.
        StdOut.setFile(forgeStormbreakerOutputFile);
        // WRITE YOUR CODE HERE TO OUTPUT TO THE OUTPUT FILE
        StdOut.print(sumOfFluxIntensityData);
    }
}
